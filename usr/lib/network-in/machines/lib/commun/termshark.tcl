...
proc fenetre_termshark {} {
  ...
  destroy .cap
  toplevel .cap
  wm title .cap  [::msgcat::mc "Frame analysis"]
  wm iconphoto .cap img_capture
  wm protocol .cap WM_DELETE_WINDOW {destroy .cap}
  ...
  $m.file add command -label [::msgcat::mc "Quit"] -command {destroy .cap}

  # menu aide
  $m add cascade -menu $m.help -label [::msgcat::mc "Help"]
  menu $m.help -tearoff 0
  $m.help add command -label [::msgcat::mc "About"] -command apropos

  # Barre de boutons
  frame .cap.fb
  pack .cap.fb -fill x
  button .cap.fb.a -compound top -relief flat -text [::msgcat::mc "Quit"] -image img_quitter -command {destroy .cap}
  ...
  ttk::button .cap.f.seecap.f3.b1 -compound left -text [::msgcat::mc "Delete capture"]  -image img_supprimer -command {
    ...
    if {$liste_sel != {}} {
        foreach n $liste_sel {
            set fic [file join "/root/captures" [.cap.f.seecap.f2.t item $n -values]]
            file delete $fic
  ...
  ttk::button .cap.f.seecap.f3.b2 -compound left -text [::msgcat::mc "Open capture"]  -image img_lire -command {
    set liste_sel [.cap.f.seecap.f2.t selection]
    if {$liste_sel != {}} {
        set fic [file join "/root/captures"  [.cap.f.seecap.f2.t item [lindex $liste_sel 0] -values]]
  ...
  ttk::button .cap.f.newcap.a -compound left -text [::msgcat::mc "Stop"] -image img_eteindre -state disabled -command {
    ...
    exec_termshark_pcap $::tmp(fic_capture)
...
proc exec_termshark_capture {interf} {
    ...
    set ::tmp(fic_capture) "/root/captures/$interf--$date.pcap"
...
proc exec_termshark_pcap {fic} {
	...
	set pid [exec st -T [wm title .]-Termshark -g 90x30+[winfo x .]+[winfo y .] -f "DejaVu Sans Mono-12" -e "/tmp/termshark_exec" &]
    # on récupère l'id de fenêtre
